...
-(void)writeString:(NSString *)string toDestination:(NSString *)destination{
    NSMutableDictionary *allHeaders = [NSMutableDictionary dictionary];
    [allHeaders setValue:destination forKey:@"destination"];
    NSMutableString *frameString = [NSMutableString stringWithString: [@"SEND" stringByAppendingString:@"\n"]];
    for (id key in allHeaders) {
        [frameString appendString:key];
        [frameString appendString:@":"];
        [frameString appendString:[allHeaders objectForKey:key]];
        [frameString appendString:@"\n"];
    }
    if (string) {
        [frameString appendString:@"\n"];
        [frameString appendString:string];
    }
    [frameString appendString:@"\n"];
    [frameString appendString:@"\0"];
    [self dequeueWrite:[frameString dataUsingEncoding:NSUTF8StringEncoding]
              withCode:JFROpCodeTextFrame];
